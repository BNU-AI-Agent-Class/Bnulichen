# s3_agent.py —— Agent 循环 + Prompt 外置到 agent.md
# 说明：系统提示词不再写死在代码里，而是从 agent.md 读取。
# Agent 会自己决定下一步：需要工具时输出 "命令:"，任务结束时输出 "完成:"。
...
# 9. 从外部文件加载 Agent 行为规则
with open("agent.md", encoding="utf-8") as f:
    agent_rule = f.read()

while True:                                                         # 10. 外层循环：等待用户输入新任务
    messages = [{"role": "system", "content": agent_rule}]          # 11. 每个新任务开始时重置上下文，只保留规则
    user_input = input("\n你：")                                     # 12. 等待用户输入
